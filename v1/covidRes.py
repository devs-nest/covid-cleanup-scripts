...
class CovidRes:
        ...
        with open(filename) as file:
            ...
            for row in read:
                try:
                    ...
                    resource_type = self.parseResourceType(
                        resource_sub_type, s, countRows)
                    if not resource_type:
                        resource_type = self.desperatelyParseResourceType(
                            row[2], countRows)

                    contact_name = self.parseContactName(row[5])
                    contact_number = self.parseContactNumber(row[6], countRows)

                    subject = str(resource_type) + ' in ' + \
                        str(state) + ', ' + str(city)
                    description = 'Contact Name: ' + str(contact_name) + '<br>Contact Number: ' + str(
                        contact_number) + '<br>Location: ' + row[4] + '<br>Source: ' + row[10]
    ...
    def getListOfWords(self, stri):
        s = [x for x in re.split('[^a-zA-Z]', stri) if x != '']
        return s

    def parseCity(self, city, rowNo):
        error = False
        if string.capwords(city) not in city_to_state:
            raise Exception('Error no city named ' + city +
                            ' exists at row no. ' + str(rowNo))
        return error, city
    ...
    def parseState(self, city, error, rowNo):
        ...
        if string.capwords(city) not in city_to_state.values():
            raise Exception('Error no state named ' + city +
                            ' exists at row no. ' + str(rowNo))
    ...
    def desperatelyParseResourceType(self, col, rowNo):
        ...
        raise Exception(
            'Error no resource type found at row no. ' + str(rowNo))
        return None
    ...
    def parseResourceType(self, stri, s, rowNo):
        ...
        if sub_types_to_type.get(stri):
            return sub_types_to_type[stri]

        if (len(s)-1) >= 0 and s[len(s)-1] in sub_types_to_type:
            # print('Found resource type as ', s[len(s)-1])
            return s[len(s)-1]
    ...
    def parseResourceSubType(self, s):
        ...
        if (len(s)-1) >= 0 and s[len(s)-1] in sub_types_to_type:
            # print('Found resource sub type as ', s[len(s)-1])
            return s[len(s)-1]
